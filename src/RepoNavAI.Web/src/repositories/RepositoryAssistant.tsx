...
import type { CodeFlowTrace, OrientationExperience, OrientationFocus, OrientationPlan, OrientationRole, RegisteredRepository, RepositoryAssistantHistoryDetail, RepositoryChatCitation, RepositoryChatEvent, SemanticSearchResult, StoredAnswerHistory, StoredSearchHistory } from './types';
...
export function RepositoryAssistant({ organizationId, repository, initialMode = 'Auto' }: { organizationId: string; repository: RegisteredRepository; initialMode?: AssistantMode }) {
  ...
  const [historyRefresh, setHistoryRefresh] = useState(0); const [historyStale, setHistoryStale] = useState(false);
  const abortRef = useRef<AbortController | undefined>(undefined);
  const promptRef = useRef<HTMLTextAreaElement | null>(null);
  const formRef = useRef<HTMLFormElement | null>(null);
  useEffect(() => () => abortRef.current?.abort(), []);
  useEffect(() => {
    let active = true; setResult(undefined); setResolvedMode(undefined); setError(''); setNotice(''); setCapabilities(undefined); setPromptSetStart(0); setHistoryStale(false);
  ...
  async function submit(event: FormEvent) {
    ...
    const controller = new AbortController(); abortRef.current = controller; setResult(undefined); setError(''); setNotice(''); setResolvedMode(undefined); setHistoryStale(false); setRunning(true);
    try {
      ...
      if (intent === 'Search') {
        ...
        if (!controller.signal.aborted) { setResult({ kind: 'Search', results: response.data }); setHistoryRefresh(current => current + 1); }
        ...
        if (!controller.signal.aborted) setHistoryRefresh(current => current + 1);
        ...
        if (!controller.signal.aborted) { setResult({ kind: 'Orientation', plan: response.data }); setHistoryRefresh(current => current + 1); }
      } else {
        ...
        if (!controller.signal.aborted) { setResult({ kind: 'CodeFlow', trace: response.data }); setHistoryRefresh(current => current + 1); }
      }
    } catch (reason) {
  ...
  function openHistory(detail: RepositoryAssistantHistoryDetail) {
    const { summary } = detail; if (!detail.result) return;
    setPrompt(summary.prompt); setMode(summary.mode); setResolvedMode(summary.mode); setHistoryStale(summary.isStale); setError(''); setNotice('');
    if (summary.mode === 'Search') {
      const stored = detail.result as StoredSearchHistory;
      setResult({ kind: 'Search', results: stored.results.map((item, index) => ({ chunkId: `${summary.id}-${index}`, path: item.path, startLine: item.startLine, endLine: item.endLine, content: '', score: item.score ?? 0, commitSha: item.commitSha, sourceUrl: item.sourceUrl })) });
    } else if (summary.mode === 'Answer') {
      const stored = detail.result as StoredAnswerHistory;
      setResult({ kind: 'Answer', answer: stored.answer, citations: stored.citations.map((item, index) => ({ number: item.number ?? index + 1, path: item.path, startLine: item.startLine, endLine: item.endLine, commitSha: item.commitSha, sourceUrl: item.sourceUrl, score: item.score ?? 0 })) });
    } else if (summary.mode === 'Orientation') setResult({ kind: 'Orientation', plan: detail.result as OrientationPlan });
    else setResult({ kind: 'CodeFlow', trace: detail.result as CodeFlowTrace });
  }
  ...
  return <div id="repository-assistant" className="min-w-0">
    ...
    <RepositoryAssistantHistory organizationId={organizationId} repositoryId={repository.id} refreshKey={historyRefresh} onOpen={openHistory}/>
    ...
    <form ref={formRef} className="mt-4" onSubmit={submit}>
      <div className="flex flex-col gap-3 md:flex-row md:items-stretch">
        <label className="field flex-1 text-xs">What do you want to understand?<textarea ref={promptRef} className="min-h-24 w-full flex-1 resize-y py-3" maxLength={2000} required disabled={isRunning} value={prompt} onChange={event => setPrompt(event.target.value)} placeholder="How does repository indexing recover after an API restart?"/></label>
        <div className="flex flex-col md:w-44">
          <label className="field text-xs">Mode<select aria-label="Assistant mode" value={mode} disabled={isRunning} onChange={event => { setMode(event.target.value as AssistantMode); setResolvedMode(undefined); }}>{['Auto','Search','Answer','Orientation','CodeFlow'].map(value => <option key={value} value={value}>{value === 'CodeFlow' ? 'Code flow' : value}</option>)}</select></label>
          {isRunning ? <button key="stop-assistant" type="button" className="button-secondary mt-3 md:flex-1" onClick={event => { event.preventDefault(); event.stopPropagation(); cancel(); }}><Square size={14}/> Stop</button> : <button key="run-assistant" type="submit" className="button-primary mt-3 md:flex-1"><Sparkles size={16}/> Ask</button>}
        </div>
      </div>
    ...
    {isRunning && <div className="mt-3"><OperationState kind="progress" title="Analyzing repository evidence" message="Working from the latest indexed commit. You can stop this request without leaving the workspace." compact/></div>}
    {notice && <div className="mt-3"><OperationState kind="stopped" title="Request stopped" message="You can update the prompt or mode and try again." compact/></div>}
    {error && <div className="mt-4"><OperationState kind="failure" title="Repository request failed" message={error} action={<RetryButton label="Retry request" onRetry={() => formRef.current?.requestSubmit()}/>} compact/></div>}
    {historyStale && <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-800">This saved result uses an older repository index. Its source links remain pinned to the original commit.</p>}
...
function CompactSelect({ label, value, values, onChange }: { label: string; value: string; values: string[]; onChange: (value: string) => void }) { return <label className="field text-xs">{label}<select className="min-h-9 px-2 text-xs" value={value} onChange={event => onChange(event.target.value)}>{values.map(item => <option key={item}>{item}</option>)}</select></label>; }
...
function CodeFlowView({ trace }: { trace: CodeFlowTrace }) { return <section id="assistant-code-flow-result" aria-label="Code flow" className="mt-6 scroll-mt-6"><div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2"><Workflow size={17} className="text-brand-600"/><h4 className="font-semibold text-ink">Execution trace</h4></div><span className="rounded-full bg-slate-100 px-2 py-1 text-xs">commit {trace.commitSha.slice(0,8)}</span></div><p className="mt-3 text-sm leading-6 text-slate-600">{trace.summary}</p><CodeFlowDiagram trace={trace}/>{trace.missingEvidence.length > 0 && <div className="mt-4 rounded-xl bg-amber-50 p-4"><p className="text-xs font-semibold uppercase text-amber-800">Missing evidence</p><ul className="mt-2 list-disc pl-5 text-sm text-amber-900">{trace.missingEvidence.map(item => <li key={item}>{item}</li>)}</ul></div>}{trace.sources.length > 0 && <details className="mt-4 rounded-xl border border-slate-200 p-4"><summary className="cursor-pointer text-sm font-semibold text-ink">Underlying retrieved evidence ({trace.sources.length})</summary><p className="mt-2 text-xs text-slate-500">Ranked source excerpts supplied to the explanation model, pinned to commit {trace.commitSha.slice(0,8)}.</p><div className="mt-3 grid gap-3">{trace.sources.map(source => <article key={source.chunkId} className="min-w-0 rounded-lg bg-slate-50 p-3"><a href={source.sourceUrl} target="_blank" rel="noreferrer" className="break-all text-xs font-semibold text-brand-700 hover:underline">{source.path}:{source.startLine}-{source.endLine}</a><pre className="mt-2 max-h-44 overflow-auto whitespace-pre-wrap [overflow-wrap:anywhere] text-xs leading-5 text-slate-600">{source.content}</pre></article>)}</div></details>}</section>; }
