...
export function RepositoryAssistant({ organizationId, repository, initialMode = 'Auto' }: { organizationId: string; repository: RegisteredRepository; initialMode?: AssistantMode }) {
  ...
  return <div id="repository-assistant" className="min-w-0">
    ...
    {isRunning && <p role="status" className="mt-3 text-sm text-slate-500">Working from the latest indexed commit…</p>}{notice && <p role="status" className="mt-3 text-sm font-medium text-slate-500">{notice}</p>}{error && <div className="error mt-4 flex flex-wrap items-center justify-between gap-3" role="alert"><span>{error}</span><button type="button" className="rounded-lg border border-current px-3 py-1.5 text-xs font-semibold" onClick={() => formRef.current?.requestSubmit()}>Retry request</button></div>}
