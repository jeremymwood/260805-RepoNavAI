...
import { CheckCircle2, Github, MinusCircle, Plus, X } from 'lucide-react';
...
export function RepositoryPanel({ organizationId, initialVisibleCount }: { organizationId: string; initialVisibleCount?: number }) {
  ...
  return <section id="repositories" className="mt-10 scroll-mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    {repositories.isLoading ? <p className="mt-6 text-sm text-slate-500">Loading repositories...</p> : repositories.data?.length ? <div className="mt-6 grid gap-3 md:grid-cols-2">{visibleRepositories?.map(repository => <RepositoryCard key={repository.id} repository={repository} selected={selected?.id === repository.id} onCancel={() => indexingAction.mutate({ repositoryId: repository.id, action: 'cancel' })} onRetry={() => indexingAction.mutate({ repositoryId: repository.id, action: 'retry' })} onExplore={() => openAnalysis(repository.id)}/>)}</div> : <div className="mt-6 rounded-xl border border-dashed border-slate-200 p-6 text-center text-sm text-slate-500">No repositories registered yet.</div>}
