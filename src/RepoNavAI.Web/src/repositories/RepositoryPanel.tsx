...
export function RepositoryPanel({ organizationId, initialVisibleCount = pageSize }: { organizationId: string; initialVisibleCount?: number }) {
  ...
  return <section id="repositories" className="panel mt-8 scroll-mt-28">
    ...
    {repositories.isLoading ? <p className="mt-6 text-sm text-slate-500" role="status">Loading repositories...</p> : repositories.isError ? <div className="error mt-6" role="alert">{getApiError(repositories.error)}</div> : totalCount ? <div id="repository-list" className="mt-6 grid gap-3 md:grid-cols-2">{visibleRepositories.map(repository => <RepositoryCard key={repository.id} repository={repository} selected={false} onCancel={() => indexingAction.mutate({ repositoryId: repository.id, action: 'cancel' })} onRetry={() => indexingAction.mutate({ repositoryId: repository.id, action: 'retry' })} onExplore={() => explore(repository.id)} retryPending={indexingAction.isPending && indexingAction.variables?.repositoryId === repository.id && indexingAction.variables?.action === 'retry'} favoritePending={favorite.isPending && favorite.variables?.repositoryId === repository.id} onFavorite={() => favorite.mutate({ repositoryId: repository.id, isFavorite: !repository.isFavorite })}/>)}</div> : <div className="empty-state mt-6">No repositories registered yet.</div>}
