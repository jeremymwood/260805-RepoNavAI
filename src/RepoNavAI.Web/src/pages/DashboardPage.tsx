...
import { ArrowRight, BookOpen, Building2, GitBranch, LayoutDashboard, LogOut, Menu, PanelLeftClose, PanelLeftOpen, Plus, Search, Settings, Sparkles, UserRound, Users, X } from 'lucide-react';
...
export function WorkspaceOverviewPage() {
  ...
  return <><section id="workspace-greeting" className="relative rounded-3xl bg-ink text-white shadow-panel"><div className="max-w-2xl"><div className="mb-5 inline-flex items-center gap-2 rounded-full border border-blue-300/20 bg-blue-300/10 px-3 py-1 text-xs font-semibold text-blue-200"><Sparkles size={14}/> {current.name}</div><h1 className="text-3xl font-semibold tracking-tight md:text-4xl">Good to see you, {(user?.displayName ?? '').split(' ')[0]}.</h1><p className="mt-4 max-w-xl text-slate-300">Turn unfamiliar codebases into navigable systems of answers, flows, and cited evidence.</p><NavLink to="/#register-repository" className="mt-7 inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-semibold text-ink transition hover:bg-blue-50">Connect repository <ArrowRight size={17}/></NavLink></div></section><section className="mt-7 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="mb-3 flex items-center justify-between"><div><p className="eyebrow">Get started</p><h2 className="mt-0.5 font-semibold text-ink">Build your understanding</h2></div><BookOpen size={20} className="text-slate-300"/></div><div className="grid gap-2 md:grid-cols-3">{actions.map(({ icon: Icon, title, copy, to }) => <NavLink key={title} to={to} className="group flex min-w-0 items-center gap-3 rounded-xl border border-slate-100 p-3 transition hover:border-brand-100 hover:bg-brand-50 focus:outline-none focus:ring-2 focus:ring-brand-500"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-brand-50 text-brand-600"><Icon size={18}/></span><span className="min-w-0"><span className="block text-sm font-semibold text-ink">{title}</span><span className="block truncate text-xs text-slate-500">{copy}</span></span><ArrowRight size={14} className="ml-auto shrink-0 text-slate-300 transition group-hover:text-brand-600"/></NavLink>)}</div></section><RepositoryPanel organizationId={current.id} initialVisibleCount={4}/></>;
}
export function ProfileSettingsPage() { const { user } = useAuth(); return <><PageHeading eyebrow="Personal settings" title="Profile" copy="Review the identity associated with this browser session."/><section className="mt-8 max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><dl className="grid gap-5 sm:grid-cols-2"><div><dt className="text-xs font-bold uppercase tracking-wide text-slate-400">Display name</dt><dd className="mt-1 font-semibold text-ink">{user?.displayName}</dd></div><div><dt className="text-xs font-bold uppercase tracking-wide text-slate-400">Email</dt><dd className="mt-1 break-all font-semibold text-ink">{user?.email}</dd></div></dl><p className="mt-6 text-sm text-slate-500">Editable profile preferences will be added when the account-management API is available.</p></section></>; }
