...
Each environment has its own resource group, virtual network, delegated PostgreSQL subnet, Container Apps environment, database, registry, vault, identities, telemetry, budget, and alerts. Environment parameter files contain capacity and retention choices but obtain the database bootstrap password from the process environment. They never contain credentials.
