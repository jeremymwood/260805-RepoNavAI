...
The browser suite uses the bundled fixture demo, so it does not require credentials, a database, or a live API. It runs axe against representative light and dark screens, verifies page containment at supported widths, and compares screenshots with committed Linux baselines. CI runs the frontend job in the pinned Playwright container so browser, font, and operating-system rendering remain deterministic. When an intentional visual change occurs, review the rendered output and run `npm run test:e2e:update` in that same container before committing updated snapshots.
